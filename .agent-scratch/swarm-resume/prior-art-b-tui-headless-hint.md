...
| Code search for `"requires an interactive terminal"` | 401 Unauthorized (code search requires auth); mitigated by grepping the local mirror `/home/bricker/src/jcode`, which tracks upstream (verified identical message and line numbers; see below) |
...
## Evidence — upstream git history (via local mirror /home/bricker/src/jcode)
...
/home/bricker/Projects/etc/hngh returns exactly one commit:
