...
probes = [
    "~/x",                       # canonical
    "cwd=~",                     # end-of-token path
    "/home",                                 # bare
    "xhome/bricker",                         # mid-token word, must stay
    "ssh://user:pass@host~/y",   # URL userinfo + path
    "//nas~/z",                  # scheme-relative mount
    "/tmp/a /root/b /Users/c/d",             # multi-family
    "/homeX/bricker",                        # word fragment, must stay
    "see~",                      # mid-token guard, must stay
    "HOME=~",                    # env-var style
    "/opt~",                     # mid-path 'home' after slash
    "macOS /Users/jane/Docs",                # Users family
    "/var/home/jo",                          # ostree-style /var/home
    "scp ~/.ssh/id_ed25519",     # credential-bearing path
]
for p in probes:
