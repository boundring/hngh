...
SEAT_UP=${SEAT_UP:-~/.local/bin/seat-up}
LANE_WATCH=${LANE_WATCH:-~/.local/bin/lane-watch}
...
if SEAT_MODEL_CATALOG=/tmp/absent-model-catalog.json \
  "$SEAT_UP" cibo openai/gpt-5.6-luna openrouter /tmp ~/.hngh-night/missions/cibo-dashboard.md \
...
import json, os, sys
path = sys.argv[1]
sys.path.insert(0, os.environ.get("HERMES_SOURCE", "~/.hermes/hermes-agent"))
