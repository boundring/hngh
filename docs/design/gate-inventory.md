...
| Session spend ceiling (env OVERNIGHT_MAX_SESSIONS_DAY > Inventory row sessions-day-max, now 200 per operator authorization 2026-09-09 > legacy constant 4) | automation/scripts/overnight-cycle.sh:47 + automation/cadence-params.tsv | JUSTIFIED |
...
| struck 2026-09-24 - :23 format-only, UNNECESSARY in effect, scoped 2026-09-17. Candidate label acceptance is format-only (no certificate ledger) | hngh tests/scripts/test-loop-history-guard.py:193,285 | UNNECESSARY in effect (upgrade scoped 2026-09-17) | The guard accepts any `hngh: candidate <64hex>` subject; kernel certificates are never persisted (single-use stdout render), so post-hoc label verification is structurally impossible and a fabricated label passes. Declared miss convention + the recon patrol's content-binding leg (c2bda901) partially cover; the full cure is a mint-time certificate receipt, adopted as Option A in docs/records/2026-09-17-certificate-ephemerality-of-record.md (proposal: docs/records/2026-09-17-candidate-reconciliation-closure.md). Orphaned-cert class (labels declared only on unreachable commits) closed 2026-09-17 as moot-by-mechanism: docs/records/2026-09-17-cert-disposition-surface-closure.md. |
| struck 2026-09-24 - :24 OBSOLETE - the cadence owns the clock. Rotation runner "operator-owned" scheduling note | hngh docs/project/queue.md scheduling section | OBSOLETE | The note predates the cadence tiers. schedule-heartbeat is now mounted (automation cadence/hour/31); queue.md should say the cadence owns the clock. |
| struck 2026-09-24 - :25 DONE - see queue row wake-mutation-lane. Wake-mutation-lane slice unimplemented | hngh docs/project/backlog.md (certificate-bound wake lane) | READY TO DISPATCH | The top queued rotation item is a full multi-slice kernel change (:wake-mutation action). Gate green, transport gap identified, recon scoped. Needs an overnight lead; not launched by this audit. |
