...
### Session M9.39: Card 99 impl + card 100 ACP flake landed (tandem B)
**Status**: Delivered + verified (2026-08-09 ~03:30–04:26, unattended tandem). Seat B (gpt-5.6-luna-max, IMPLEMENT lane) implemented both cards from A's spec; docs + count refs closed by A (this session).
- **Card 99 `c657071`** — `:operation` human gate per `docs/design/operation-gate.md` (A's spec): `*approved-operations*` + lock (config-seeded `:operation-approvals` fail-soft), `submit-task operation-spec` forces `:type :operation` + `:authority :approval` (re-set after the v3 flatten — seam trap from `f343128`), unapproved submit → `:blocked` "awaiting-human-approval", `approve-task` human-only (flips :blocked→:queued), `operation-gate-check` exact-match + lint-deps composition (approved-but-lint-failed REFUSED), driver pre-delegate gate (refusal → :failed, no delegate), package-manager gates x3 before any daemon call (soft funcall; orchestrator absent → refuse). Suite `:hngh.operation-gate` 8 tests / 38 checks.
- **Card 100 `51ebf47`** — ACP pipe flake root-caused: vendored `jsonrpc:client-disconnect` calls `bt2:destroy-thread` (generic, no applicable method under pinned qlot) → leaked reader threads → JSONRPC-PARSE-ERROR killed the image on recycled fds. Fixed `acp-disconnect` with direct `bt:destroy-thread` + `receive-message` stream-error→NIL (clean EOF). 0 leaked threads verified.
- **A-lane draws (this session)**: CHANGELOG entries (3784f93, cards 99+100), next.md item-8 row DONE + count refs (6950a75), work-sessions M9.38/M9.39, coord.lisp MCP yason fix committed `9d5f23b` (prior-incarnation work, verified green on disk at 956/956).
- **Verified** (real output, 04:26): `make test-fast` exit 0, **19 suites, 956 checks, 0 fail** (clean /tmp); `make lint-counts` OK after ref update. C6 remains PARKED until Wave C fully ships.

### Session M9.38: Card 99 — :operation human gate design (tandem Seat A)
**Status**: Design delivered + committed (2026-08-09 03:02–03:53, unattended tandem). Seat A (deepseek-v4-flash-0731 via openrouter, DESIGN lane) extended the `:operation` task type to core-file commits + dep installs for Wave C item 8; Seat B (gpt-5.6-luna) implemented it (committed `c657071`, impl record M9.39).
- **Surface verified in code, no guesses**: `:operation` exists in `*task-types*` (ai-orchestrator.lisp:786) but nothing submits it; the only approval enforcement in the queue is `next-eligible-task` gating `:authority :approval` on `:approval-at > 0` (lines 1317-1319) — with NO setter anywhere. `submit-task` v3 merge exists; `task-driver-tick` delegate is the commit-producing step; package-manager `install-packages` (329) is the real dep-install entry.
- **Design `docs/design/operation-gate.md`** (committed `8933f89`): `operation-spec` payload on submit-task → `:type :operation` + `:authority :approval`; unapproved submit → `:blocked` "awaiting-human-approval" + journaled `:denied` + `operation.denied` event; `approve-task` human-only (config-seeded `:operation-approvals` durable + live registry); `operation-gate-check` exact-match predicate composing lint-deps for `:core-commit`; driver pre-delegate gate; package-manager gate in the 3 privileged ops; 8 fixture tests.
- **Seam trap caught pre-impl** (`f343128`): submit-task line 1181 force-flattens ANY v3 record's `:authority` to `:worker`, which `next-eligible-task` does NOT gate — an operation task would have dispatched without the approval gate. Spec requires re-setting `:authority :approval` after the v3 merge; B's impl confirmed fixed.
- **Deployment note** (`1ca1edf`): `:operation-approvals` lives in config/hngh.lisp which safety-boundary 0444-locks — owner `config-set` needs chmod first (same quirk as `:tool-grants`).
- **Review of B's impl (read-only)**: `:hngh.operation-gate` suite verified green in isolation — 38 checks, 0 fail (03:52). One compile-blocker STEERed + resolved (unbalanced test-operation-gate.lisp; B re-saved balanced 03:47). Impl remained uncommitted at Seat-A wind-down (03:53); docs row + count refs queued for the next session after B's commit.
- Verified: `make test-suite SUITE='(:hngh.operation-gate)'` exit 0 (38/38); `:hngh.coord` exit 0 (10/10). Coordination: one `make test` at a time (two concurrent SBCL runs collide — observed 7 spurious fails 03:10).

### Session M9.37: Tandem delivered Wave C item 3; ACP director built
**Status**: Delivered + verified (2026-08-09 ~01:15–02:10). Tandem experiment succeeded; card 97 landed.
- **The tandem worked, unattended**: Seat A (deepseek-v4-flash) wrote the design spec `docs/design/least-agency-scoping.md` (commit `2244281`) and — before B finished — wrote detailed STEER: notes (three real test bugs: stale `grants-path` symbol, wrong bus topic `agent.denied` vs `tool.denied`, a persistence test contradicting the spec). Seat B (gpt-5.6-luna) implemented deny-by-default `*tool-grants*` (registry, granted-p, grant/revoke, read-only auto-grant, denial journaling, `tool.denied` bus event, select-tool filter), 430 insertions incl. 311-line test-scoping.lisp, committed `dccad77`, wrote FINAL-B.md (01:36), stopped itself per wind-down. **904/904 fast, 0 fail-suites, verified after the fact.**
- **Infrastructure built along the way (owner-directed)**: `tandem-director` (Python, stdio JSON-RPC ACP client: initialize → session/new → session/prompt, content-block payloads, request_permission auto-deny gate with AUTODENY regex list, stall policy, dashboard keys a/b/x/r/q/h); `tandem-tui` (llmtrim-style conversation renderer reading the director's JSONL); `tandem-log-view`; `tandem-acp-up` launcher (top-aligned cascade windows); systemd-unit option for director decoupling (lifecycle: seats are children of director; a window close kills the tree — noted, unit option preferred next run).
- **Protocol facts learned (from source, not guessing)**: `hermes acp` takes `-m/--provider` BEFORE the subcommand; caps served under `agentCapabilities`; `session/new` REQUIRES `mcpServers: []`; `session/prompt` takes a LIST of content blocks; `request_permission` answers with `AllowedOutcome.option_id` from a fixed option set.
- **Owner feedback mid-run (recorded)**: (a) views should be conversation-rendered (llmtrim-style TUI), not raw JSON; (b) pane layout preferred — two/three panes in one konsole window (tmux-style) rather than separate windows; (c) viewing seats in a Hermes TUI session is NOT possible while ACP controls them (same session store, two writers) — window renders conversation, director keeps control.
- **Own failures owned**: bounced a productive seat on a self-report (read the shared state first); misread a grep as "everything died" (both seats were fine); truncated seat logs (300-char cap) killed TUI rendering — fixed to pure JSONL in the current director.

### Session M9.36: Two-Hermes tandem experiment launched (card 97)
**Status**: Launched + running (2026-08-09 ~01:15). Owner supervised; seats wind down ~06:00 or on completion.
- **Idea (owner, M9.35→36)**: drop role specs; let two Hermes agents on different models collaborate free-form, steering each other — tandem coding. Two Hermes seats steer each other via ACP (both expose `hermes acp`); work handoff via inbox/outbox files.
- **Launch**: `tandem-up` launcher — Seat A = deepseek-v4-flash-0731 (openrouter) DESIGN lane (docs/design/least-agency-scoping.md only); Seat B = gpt-5.6-luna (openai) IMPLEMENT lane (ai-tool-hub/sentry/packages/tests/Makefile/asd). Lane separation by file scope — no overlapping files, so no git collision (the line-range split I first drafted would have drifted as commits landed; fileName-scope lanes avoid that). Shared `~/.hngh-night/tandem-20260809/` inbox/outbox/worklog; STEER: notes allowed cross-lane; poll-don't-block. Both windows in Konsole cascade (`--geometry` offset), DISPLAY :0 verified.
- **Wind-down**: seats check `date`; gentle halt ~06:00 with FINAL-A/B.md if incomplete; done-early → stop with summary (no invented work). Commits local-only (never push), attribution tags tandem-a/tandem-b. Honesty rules on test output.
- Docs: next.md tandem block. Session stays attended/supervised; steering available via ACP if seats stall.
