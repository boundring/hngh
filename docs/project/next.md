...
- **Wave C item 3 — native least-agency tool scoping, TANDEM-DELIVERED** (`dccad77`): seat B (gpt-5.6-luna, ACP) implemented deny-by-default `*tool-grants*` in the AI tool hub from seat A's (deepseek-v4-flash) spec `2244281` — grants registry + `tool-granted-p` + `grant-tool`/`revoke-tool`, read-only tools auto-granted, denials journaled + `tool.denied` bus event, `select-tool` filter; 46 new checks (914/914 fast, 0 fail-suites). **The two-Hermes ACP tandem experiment produced and verified a full Wave C gate item unattended** (~35 min launch→commit→FINAL), with flash-as-review-counterpoint STEER: notes folded in mid-flight. All Wave C gate items 1–4 now land; `:operation` gate (99) + canary/scan tail remain.
...
| P0 | **Wave C security baseline** — immutable safety layer (part 1 shipped `fd5bc82`); **item 1 qlot pin DONE** (`cb7b82f`), **item 4 hash-chain DONE** (Luna-delegated, 6441e0a), **item 2 bwrap sandbox DONE** (6441e0a, run-sandboxed default-deny + tool-hub `sandboxed-p` flag); adoption research done + reviewed (OPA shelved ADR-044; Canarytokens, LLM Guard to adopt); remaining: **item 3 native least-agency scoping** (no OPA), canary+scan sidecars, `:operation` gate (no core self-modification until 1–4 land; 1,2,4 land — item 3 open) | hngh; `docs/design/autonomy-strategy.md` §7 |
