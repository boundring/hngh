...
| P0 | **Wave C security baseline** — immutable safety layer (part 1 shipped `fd5bc82`: protected configs, fail-closed guard, append-only action log); **adoption research done + reviewed** (`docs/research/wave-c-open-source-tooling.md`: OPA shelved ADR-044; Bubblewrap, qlot, Canarytokens, LLM Guard to adopt; hash-chained log via hermes #487 pattern); remaining: qlot pin, bwrap sandbox, native least-agency scoping (no OPA), hash-chaining, canary+scan sidecars, `:operation` gate (no core self-modification until 1–4 land) | hngh; `docs/design/autonomy-strategy.md` §7 |
