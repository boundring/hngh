...
| P0 | **Wave C security baseline** — immutable safety layer (part 1 shipped `fd5bc82`: protected configs, fail-closed guard, append-only action log); remaining: least-agency tool scoping, untrusted-content tagging, canary tokens, execution sandboxing, pinned deps, `:operation` human gate for core-file commits + dep installs (gate: no self-modification of core until complete) | hngh; `docs/design/autonomy-strategy.md` §7 |
