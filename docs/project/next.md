...
| P0 | **Wave C security baseline** — immutable safety layer (part 1 shipped `fd5bc82`: protected configs, fail-closed guard, append-only action log); **adoption research done** (`docs/research/wave-c-open-source-tooling.md`: OPA, Bubblewrap, qlot, Canarytokens, LLM Guard, hash-chained log via hermes #487 pattern); remaining: qlot pin, bwrap sandbox, OPA scoping, hash-chaining, canary+scan sidecars, `:operation` gate (no core self-modification until 1–4 land) | hngh; `docs/design/autonomy-strategy.md` §7 |
