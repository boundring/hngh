...
| P0 | **Wave C security baseline** — immutable safety layer (part 1 shipped `fd5bc82`: protected configs, fail-closed guard, append-only action log); **item 1 qlot pin DONE** (`qlfile`+`qlfile.lock` pin Quicklisp 2026-01-01 + deps incl. ironclad; Makefile loads `.qlot/setup.lisp` when present; CI provisions via qlot); **item 4 hash-chain DONE** (SHA-256-chained action log + `verify-action-log` fail-closed, `c4c5b88`); adoption research done + reviewed (OPA shelved ADR-044; Bubblewrap, Canarytokens, LLM Guard to adopt); remaining: bwrap sandbox, native least-agency scoping, canary+scan sidecars, `:operation` gate (no core self-modification until 1–4 land) | hngh; `docs/design/autonomy-strategy.md` §7 |
