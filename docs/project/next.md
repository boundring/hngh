...
- **Wave C item 1 — qlot dep pinning + CI red→green** (`cb7b82f`): `qlfile`+`qlfile.lock` (Quicklisp 2026-01-01 + bordeaux-threads/cl-ppcre/babel/jsonrpc/alexandria/yason/jsown/fiveam); Makefile `SBCL_FLAGS` loads `.qlot/setup.lisp` when present (no-op without qlot); `.qlot/` gitignored; **`ci.yml` rewritten** to install SBCL+Quicklisp+qlot and `qlot install` + `qlot exec make build/test` (was failing every push — no Quicklisp on runner); **`mirror.yml` deleted** (redundant — local already pushes both remotes; + it failed on runner). 837/837 fast, 2578/2578 full still green under the pin.
...
| P0 | **Wave C security baseline** — immutable safety layer (part 1 shipped `fd5bc82`: protected configs, fail-closed guard, append-only action log); **item 1 qlot pin DONE** (`qlfile`+`qlfile.lock` pin Quicklisp 2026-01-01 + 8 deps; Makefile loads `.qlot/setup.lisp` when present; CI provisions via qlot); adoption research done + reviewed (OPA shelved ADR-044; Bubblewrap, Canarytokens, LLM Guard to adopt; hash-chained log via hermes #487); remaining: bwrap sandbox, native least-agency scoping, hash-chaining, canary+scan sidecars, `:operation` gate (no core self-modification until 1–4 land) | hngh; `docs/design/autonomy-strategy.md` §7 |
