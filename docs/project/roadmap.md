...
- Completed the dogfood development loop (promotion rung 9, 2026-08-24): the operator governance surface (`propose`, `issue-cert`, `mutation-check` in `scripts/hngh`) forms closed policy proposals, mints candidate certificates under admitted verdicts, and executes the certificate-bound mutation against real repository evidence including live base revision, per-file content hashes, and the installed verify-candidate script. The first self-governed commit — this documentation change — was produced, reviewed, and committed by Hngh under its own certificate and pushed to origin.
1. **Bounded agent worker transports.**

## Next

1. **Bounded agent worker transports.** Admit real model and terminal transport implementations behind explicit run loadout configurations.
2. **Distributed attestation & evidence federation.** Paper-first design for remote evidence ports and cross-machine certificate verification.
