# Do any files in `/home/bricker/Projects/etc/hngh` contain the strings `LD_PRELOAD`, `getenv("HNNGH_")`, or a directory named `stubs/`?
...
**Research line:** Do any files in `/home/bricker/Projects/etc/hngh` contain the strings `LD_PRELOAD`, `getenv("HNNGH_")`, or a directory named `stubs/`?
...
1. **Execute the probe contract on an idle host** with read-only access to `/home/bricker/Projects/etc/hngh`. The exact commands and properties are laid out in the prior material (see References). Do not substitute a narrower search.
...
- **Scope boundary.** The line is scoped to `/home/bricker/Projects/etc/hngh` only. If the mechanism under investigation spans a sibling repository (e.g., `hngh-automation`), that would be a separate line. The prior material references both names; this record does not conflate them.
...
- **Target path:** `/home/bricker/Projects/etc/hngh` — the subject of investigation. I cite this path as the designated scope; I do not assert that any specific file or directory within it exists.
...
No file paths within `/home/bricker/Projects/etc/hngh` are cited as existing, because I have no access to that filesystem and cannot confirm any specific path. Any such citation would be an unverified assertion, which this record explicitly avoids.
