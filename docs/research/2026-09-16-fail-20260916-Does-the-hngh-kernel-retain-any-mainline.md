...
These are the bounded, executable actions for hngh-automation on an idle host with read access to `/home/bricker/Projects/etc/hngh`. They are ordered by dependency; each stage gates the next.

### R1 — File-presence probe (gate)

```bash
for f in mm/oom_kill.c mm/oom_v2.c mm/vmscan.c; do
  test -f "/home/bricker/Projects/etc/hngh/$f" && echo "PRESENT $f" || echo "ABSENT  $f"
done
```
...
grep -n 'oom_kill' /home/bricker/Projects/etc/hngh/mm/Makefile

# 2. Is there a Kconfig gate?
grep -rn 'OOM_KILL\|oom_kill' \
  /home/bricker/Projects/etc/hngh/mm/Kconfig \
  /home/bricker/Projects/etc/hngh/init/Kconfig 2>/dev/null

# 3. What does the active .config say? (if a build tree exists)
grep -E '^CONFIG_OOM_KILL' /home/bricker/Projects/etc/hngh/.config 2>/dev/null \
  || grep -E '^CONFIG_OOM_KILL' \
     /home/bricker/Projects/etc/hngh/arch/x86/configs/*defconfig 2>/dev/null
```
...
```bash
SYMFILE=$(find /home/bricker/Projects/etc/hngh -maxdepth 2 \
...
grep -rn 'out_of_memory\|oom_kill_process\|__alloc_pages.*__GFP_NORETRY' \
  /home/bricker/Projects/etc/hngh/mm/ --include='*.c' --include='*.h'
```
...
The entire question remains open pending a single shell session against `/home/bricker/Projects/etc/hngh` on a host with read access. No further design work is needed. This thread reopens the line if and when that observation is made.
...
The following are the concrete references grounding this crystallization. File paths under `/home/bricker/Projects/etc/hngh/` are cited as **expected locations in a Linux-derived kernel tree**, not as confirmed-present files; their existence is the subject
