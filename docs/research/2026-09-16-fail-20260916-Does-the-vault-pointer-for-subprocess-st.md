...
**Scope of evidence:** This repository's provided prior material only. I have **no shell/filesystem access** to `~/Projects/etc/hngh` or to any vault directory; every on-disk claim below is therefore either grounded in the supplied text or explicitly marked unverified.
...
2. **Map "seam" to kernel code (optional, to confirm F3).** In `~/Projects/etc/hngh`:
...
- **O3 — Kernel-repo implementation status of the seam.** Whether an env-overridable binary stub actually lives in `~/Projects/etc/hngh` is unverified (no code cited or inspectable here).
...
I cannot verify any external or on-disk source from this environment: I have no access to `~/Projects/etc/hngh`, to any `llm-wiki` vault, or to the filesystem that would host `/path/to/…`. All statements about *file existence* are therefore either (a) grounded in the provided prior-art text, or (b) explicitly labeled inference/unverified. I do **not** assert that any file exists at a real absolute path.
...
3. **hngh kernel repository root (as given in the task context; contents not inspected here):** `~/Projects/etc/hngh`.
