...
On any idle host with access to `~/Projects/etc/hngh`:

```bash
test -d ~/Projects/etc/hngh/hngh-automation && echo "EXISTS" || echo "ABSENT"
```
...
```bash
find ~/Projects/etc/hngh/hngh-automation -type f -o -type l | sort
grep -rn "R1" ~/Projects/etc/hngh/hngh-automation/ 2>/dev/null
grep -rn "capture" ~/Projects/etc/hngh/hngh-automation/ 2>/dev/null
```
...
If the observation was generated by an agent run, check whether the agent's working directory matches `~/Projects/etc/hngh` and whether the build log references `hngh-automation` as a path or a logical name.
...
```bash
find ~/Projects/etc/hngh -type d -name "*hngh-automation*" 2>/dev/null
```
...
|--------|--------|--------------------|
| Binary existence of `~/Projects/etc/hngh/hngh-automation` | **Unresolved** | Requires filesystem access on an idle host; no prior beat has recorded a probe result. |
...
- `~/Projects/etc/hngh` — Target repository path. Cited in the line definition and prior material. I cannot verify its current state from this position.

**Explicit non-verification:** I have not run any command against `~/Projects/etc/hngh`. I have not read any file within that repository. All claims about the directory's existence, contents, or absence are conditional on a probe that has not yet been executed and recorded. No claim in this summary asserts filesystem state as fact; all such statements are framed as "requires verification" or "unresolved."
