# Does the file structure at /home/bricker/Projects/etc/hngh contain a JSON or YAML record with keys `parse_pass` and `operator_override` for the run tag 2026-09-09-stall-recovery-and-operator-surfaces?
...
1. **Execute the Verification Pass:**
   - On any host with read access to `/home/bricker/Projects/etc/hngh`, run the following command block to verify the existence of the `parse_pass` and `operator_override` keys for the run tag `2026-09-09-stall-recovery-and-operator-surfaces`:
     ```bash
     ...
     grep -rE '"(parse_pass|operator_override)"\s*:' /home/bricker/Projects/etc/hngh --include='*.json' --include='*.yaml' --include='*.yml' -l

     # 3. Cross-reference: confirm both keys co-occur in the same document
     for f in $(grep -rl '2026-09-09-stall-recovery-and-operator-surfaces' /home/bricker/Projects/etc/hngh --include='*.json' --include='*.yaml' --include='*.yml' 2>/dev/null); do
       if grep -q '
