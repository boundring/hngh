...
- The only two paths this record can name with confidence are the ones given in the line-state header itself: `research-lines.tsv` (the line-state file, referenced in the prompt header) and `~/Projects/etc/hngh` (the kernel repository root, named in the prompt). Their contents remain **[unverified]**.
...
- `~/Projects/etc/hngh` — hngh kernel repository root, named in the research prompt; no contents were read on this line.
