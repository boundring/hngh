...
if [ -x "$HNGH_HOME/scripts/dashboard-readout" ] || [ -x ~/Projects/etc/hngh/scripts/dashboard-readout ]; then
  HNGH="${HNGH_HOME:-~/Projects/etc/hngh}"
  ...
  tmp="~/Projects/etc/hngh-automation/dashboard/.readout.json.$$.tmp"
  if (cd "$HNGH" && python3 scripts/dashboard-readout --json \
    >"$tmp" 2>/dev/null); then
    mv "$tmp" ~/Projects/etc/hngh-automation/dashboard/readout.json
