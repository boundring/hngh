...
- `lib/model.sh` MODEL_PIN routing (2026-09-07, operator quota directive):
  ...
  | `kimi` | kimi -> unsloth -> ollama -> deck -> archive; remote skipped | research beat every Nth run (`kimi-research-share`, default 3); research REVIEW transition always; fresh-eyes review (`04-review-prep.sh`) always |
  | `deck` | deck -> unsloth -> ollama -> kimi -> archive; remote skipped | (available for second-server overflow; no cycle pins it yet) |
