...
| dashboard-logs 1: "rerenderWithOpState and rerenderOp call renderLogs without checking lastRender.res — runtime error when operator state updates before the initial spine load" (routed candidate 2026-09-10) | PARKED (premise stale) | Unreachable in the served sources: both entry points early-return on `lastRender.d === undefined` (dashboard/app.js :154, :162) before any render call, so the pre-load window routes to the known empty state — the behavior the finding asks for; d and res are assigned atomically in renderLogs (:601), so no state has d defined with res undefined; the renderLogs chain (operatorItemsHtml/renderReports/renderDigest/logsSum) consumes no res; the sole res consumer renderHeader is call-site guarded (:155). A literal `if (!lastRender.res)` skip would break the op-items re-render these functions exist for. The finding's own check (pre-load op-state update does not crash) passes — contract pinned in tests/test-dashboard-p0.py (OpRerenderPreloadSafety). |
