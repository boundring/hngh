...
# commits carry the operator identity boundring <boundring@gmail.com> explicitly
# (2026-09-24 operator attribution decision), set per invocation so attribution
# never depends on ambient .git/config state
# (the 2026-09-13 Fixture-leak class: 564 misattributed commits).
committed=0
if ! git -C "$repo" diff --cached --quiet; then
 ts="$(date '+%Y-%m-%d %H:%M:%S')"
 git -C "$repo" -c user.name="boundring" -c user.email="boundring@gmail.com" \
