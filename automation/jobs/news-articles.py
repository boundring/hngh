...
and committed under ~/.hngh/newspaper/<date>/articles/<slug>.md
(operator userspace-home policy 2026-09-13: newspaper artifacts never
live in the repo). Built FROM the item data (headline, severity tag,
source URL) plus the linked source page when fetchable (ONE bounded GET,
10s timeout, meta description / first paragraph only; failure -> article
from wire data alone). Fabrication is
...
newspaper-articles-cap); token cap per session via
newspaper-article-budget (default 1024, env NEWS_ARTICLES_BUDGET).
Paid-cost conversion 2026-09-13: every session pins the LOCAL model leg
(unsloth/ollama) -- the kimi/ocgo/remote quota legs are never touched by
this lane; per-edition usage (sessions, tokens in/out, paid calls=0)
accumulates in LAST_USAGE for the overnight edition driver. Top 3
articles per edition get ONE image each via jobs/imagegen-submit.sh
(style row news-illustration, 768x512) into <date>/media/ beside the
articles -- fail-closed: no image when the leg is down. Seams (tests,
hermetic): HNGH_AUTOMATION_ROOT, HNGH_NEWSPAPER_DIR, NEWS_ARTICLES_CAP,
NEWS_ARTICLES_BUDGET, NEWS_ARTICLES_MODEL_CMD,
...
DIGESTS = (os.environ.get("HNGH_DIGESTS_DIR")
           or (hngh_home().digest_dir() if os.path.isfile(
               os.path.join(ROOT, "lib", "hngh_home.py"))
               else os.path.join(ROOT, "digest")))
...
def pick_pin():
    """MODEL_PIN: env override wins (tests); else always the LOCAL leg
    (unsloth -> ollama). The paid quota legs (kimi/ocgo/remote) are
    never touched by this lane (operator cost directive 2026-09-13)."""
...
LAST_USAGE = {"calls": 0, "tokens_in": 0, "tokens_out": 0, "paid": 0}
...
def model_reply(prompt, pin, budget=MODEL_BUDGET):
    ...
    if not r.stdout.strip() or used.endswith("archive-only"):
        return ""
    if used:  # per-edition telemetry: the winning leg's usage counters
        LAST_USAGE["calls"] += 1
        for key, path in (("tokens_in", "tmp-tokensin.txt"),
                          ("tokens_out", "tmp-tokensout.txt")):
            try:
                LAST_USAGE[key] += int(
                    open(os.path.join(ROOT, path)).read().strip() or 0)
...
def make_image(subject, date):
    """ONE scene illustration via jobs/imagegen-submit.sh (style
    news-illustration, 768x512) into <date>/media/ beside the articles
    (userspace-home policy). Returns the edition-relative path
    ("media/<file>") or None (fail-closed: leg down = headline-only)."""
    cmd = os.environ.get("NEWS_ARTICLES_IMAGEGEN_CMD") or IMAGEGEN
    out_dir = os.path.join(ARTICLES, date, "media")
    os.makedirs(out_dir, exist_ok=True)
    try:
        r = subprocess.run(
            [cmd, "--style", "news-illustration", "--subject", subject,
             "--out-dir", out_dir], capture_output=True, text=True,
            timeout=180)
    except (OSError, subprocess.TimeoutExpired):
        return None
    m = re.search(r"wrote (\S+\.png)", r.stdout + r.stderr)
    return "media/%s" % os.path.basename(m.group(1)) if m else None
...
def write_article(date, item, body, image, used, profile=None):
    """Commit one article under <paper>/<date>/articles/<slug>.md.
    ...
    d = os.path.join(ARTICLES, date, "articles")
...
def generate(date, digest_path=None):
    ...
    article paths. LAST_USAGE accumulates the edition's model-call
    telemetry (local legs only; paid stays 0)."""
    LAST_USAGE.update({"calls": 0, "tokens_in": 0, "tokens_out": 0,
                       "paid": 0})
    ...
    budget = os.environ.get("NEWS_ARTICLES_BUDGET") or \
        get_param("newspaper-article-budget", MODEL_BUDGET)
    dh = digest_html()
    with open(digest_path, encoding="utf-8") as f:
