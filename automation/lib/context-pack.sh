...
# state digest (ceremony-optimization 2026-09-24): one line carrying the
# facts a session otherwise burns orientation tokens re-deriving (bead
# counts, queue/roadmap Next). Fail-open: on ANY error the output is
# empty and the pack omits the line silently.
context_state_digest() { # -> one line on stdout, empty on failure
 local root="${ROOT:-${AUTOMATION_ROOT:-}}"
 [ -n "$root" ] && [ -f "$root/jobs/city-state.py" ] || return 0
 python3 "$root/jobs/city-state.py" 2>/dev/null | python3 -c '
import json
import sys

try:
    d = json.load(sys.stdin)
    print("state digest: beads %s open/%s closed | queue next: %s | roadmap next: %s"
          % (d["beads"]["open"], d["beads"]["closed"],
             d["orient"]["queue_next"], d["orient"]["roadmap_next"]))
except Exception:
    pass
...
# timestamp+role header, state digest (fail-open), repo map, ledger
# paths, role hint, frontier
